...
{
	int fd;
	int dst_addr, src_addr;

	if(argc != 5){
		printf("***  $ srec_to_img.exe <dst_addr> <src_addr> <srec_file> <img_file>\n");
		return 0;
	}
	f_mgr_prg_buf_addr_valid = 0;
	if(s_file_loader(argv[3])){
	...
	dst_addr = get_numb(argv[1]);
	src_addr = get_numb(argv[2]);
	
	printf("=== dst_addr=%x\n", dst_addr);
	printf("=== src_addr=%x\n", src_addr);
	fflush(stdout);
	
	memmove(&ram_buffer[dst_addr], &ram_buffer[src_addr], 1440*1024);
	
	fd = _open(argv[4], O_WRONLY|O_BINARY|O_CREAT|O_TRUNC, 0777);
	if(fd < 0){
		printf("*** output file open error\n");
		return 0;
	}
	_write(fd, &ram_buffer[0x7e00], 1440*1024-1024);
